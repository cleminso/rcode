...
- [Jazz schema workflow](#jazz-schema-workflow)
- [UI package rules](#ui-package-rules)
...
- Root scripts:
  - `pnpm dev` runs recursively across app/package workspaces.
  - `pnpm build` runs recursively across workspaces.
  - `pnpm jazz:deploy` deploys Jazz schema and permissions using `apps/web/.env`.
  - `pnpm jazz:migrations:create <appId> --fromHash <fromHash> --toHash <toHash>` creates a Jazz migration.
  - `pnpm jazz:migrations:push <appId> <fromHash> <toHash>` pushes a Jazz migration using `apps/web/.env`.
...
- Follow `specs/workflow/schema-and-perms-changes.md` whenever `packages/database/schema/src/schema.ts` or `packages/database/schema/src/permissions.ts` changes.
- Before marking schema or permission changes complete, run the Jazz deploy workflow or explicitly state why it was not run.
- If `pnpm jazz:deploy` reports that schema hashes are not connected, create, inspect, rename, push, and redeploy the migration before continuing.
- Commit migration files under `packages/database/schema/migrations`; do not commit accidental migrations under app directories.
